...
cd ahk
./AutoHotKeyU64.exe ../install_demo.ahk &
...
echo "#!/bin/sh
cd ahk 
./AutoHotKeyU64.exe ../run.ahk \$@ > \$LOG_FILE" > worldinconflict

echo "#NoEnv

; Run the benchmark once to create the configuration directory
RunWait, C:\Program Files (x86)\Sierra Entertainment\World in Conflict - DEMO\wic.exe -dx9 -benchmarksaveandexit" > install.ahk
...
FileAppend, myWaterReflectClouds 0`n, %cfgfile%

; Finally run the benchmark
RunWait, %wic_path%\wic.exe -dx9 -benchmarksaveandexit

FileRead, ret, %mydocs%\World in Conflict - DEMO\fpsdata.txt
FileAppend, %ret%`n, *' > run.ahk
